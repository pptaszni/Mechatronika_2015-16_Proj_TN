...
#define pozycjaneutralna 10
#define pozycjadojazdyszybkiej 0
#define pozycjazwiekszonegodocisku 15

#define slabehamowanie 30
#define sredniehamowanie 60
#define mocnehamowanie 90
/*---------------------------*/
...
volatile unsigned int stanEnkodera;
bool docisk=0;
enum silaHamowania{
  wogole=0b0,
  troche=0b11,
...
void liczEnkoder(){//liczenie sygnalow z enkodera
    delay(1);
    if(digitalRead(enkoderA)) return;
    
    if(koncowaPozycja>stanEnkodera){
      stanEnkodera++;
      return;
    }
    if(koncowaPozycja<stanEnkodera){
...
bool enkoderVSpwm(){//wysyla do serwa sygnal zalezny od aktualnej pozycji
  if(stanEnkodera==koncowaPozycja){
    Serwo1.write(90);
    return 0;
  }  
  if(koncowaPozycja>stanEnkodera){
...
bool sterowanieSpojlerem(){
  if(docisk){
    koncowaPozycja=pozycjazwiekszonegodocisku;
    return 0;
  }
...
bool ustawSpojler(pozycjaSpojlera poz){
  if(poz==neutralna){
    koncowaPozycja=pozycjaneutralna;
    return 0;
  }
  if(poz==doJazdySzybkiej){
    koncowaPozycja=pozycjadojazdyszybkiej;
    return 0;
  }
  if(poz==hamowanie){
    if(jakMocnoHamujemy==troche){
      koncowaPozycja=(slabehamowanie);
      return 0;
    }
    else if(jakMocnoHamujemy==srednio){
      koncowaPozycja=(sredniehamowanie);
      return 0;
    }
    else if(jakMocnoHamujemy==wgniatamywpodloge){
      koncowaPozycja=(mocnehamowanie);
      return 0;
    }  
  }
  else{
...
void setup(){
  ...
  pinMode(przycisk,INPUT_PULLUP); //przycisk docisku
  pinMode(serwo1,OUTPUT); //serwo
  attachInterrupt(digitalPinToInterrupt(enkoderA), liczEnkoder, CHANGE);//funkcja liczEnkoder() wywolywana przez zewn. przerwanie na pinie 2
  attachInterrupt(digitalPinToInterrupt(przycisk), zwiekszonyDocisk, FALLING);//przycisk stalego docisku
}
void loop(){
